...
document.querySelector("#btn1").addEventListener("click", () => {
    ...
    // 1. 부모 노드 찾기 (요소.parantNode) -> li1 부모 노드 배경색 변경하기
    li1.parentNode.style.backgroundColor = "skyblue";

    // 2. 첫번째 자식 노드 찾기 (요소.firstChild) -> #test(ul태그)의 첫번째 자식 노드 찾기
    console.log(test.firstChild);

    // 3. 마지막 자식 노드 찾기 (요소.lastChild) -> #test(ul태그)의 마지막 자식 노드 찾기
    console.log(test.lastChild);

    // 4. 원하는 위치(index)에 존재하는 자식 노드 찾기 (요소.childNodes[index])
    //      -> #test(ul태그)의 자식 노드중 3번쨰 li 태그를 찾아서 배경색 pink 지정하기

    test.childNodes[9].style.backgroundColor = "pink";

    // 5. 이전/다음 형제 노드 찾기 (이전형제노드: 요소.previousSibling, 다음형제노드: NextSibling)
    //      -> 탐색용 코드는 연달아서 작성 가능
    // test.childNodes[9]의 다음 다음 형제 요소 선택
    console.log(test.childNodes[9].nextSibling.nextSibling);

    // #test의 마지막 자식 노드의 이전 형제
    ...
    // 1. 마지막 자식 노드로 추가(appendChild(노드)) -> list[11]의 마지막 자식으로 "zzz" 라는 TextNode 추가
    ...
    list[11].prepend("hello");  // 마지막에 작성 -> 첫번째 자식(맨 앞) 추가
